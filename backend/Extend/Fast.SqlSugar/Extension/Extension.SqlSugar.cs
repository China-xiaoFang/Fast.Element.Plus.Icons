...
public static class Extension
{
        ...
        where TEntity : class, new()
    {
        ...
        // 默认Db
        var defaultDb = _db.GetConnection(SugarGlobalContext.ConnectionStringsOptions.DefaultConnectionId);
        ...
        switch (dbType.DbType)
        {
            ...
            case SysDataBaseTypeEnum.Tenant:
                ...
                {
                    // 获取租户Id
                    tenantId = SugarGlobalContext.GetTenantId();
                }

                // 这里每次都获取一下数据库信息，方便跨库连接
                SugarGlobalContext.TenantDbInfo =
                    SqlSugarClientHelper.GetDbInfo(defaultDb, SysDataBaseTypeEnum.Tenant, tenantId.ParseToLong());

                return SqlSugarClientHelper.GetSqlSugarClient(_db, SugarGlobalContext.TenantDbInfo);
            default:
