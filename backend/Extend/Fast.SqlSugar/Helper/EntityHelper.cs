...
public static class EntityHelper
{
    ...
    /// 缓存 Entity 类型
    /// </summary>
    private static List<SugarEntityTypeDto> _cacheEntityTypeList { get; set; }
    ...
    /// </summary>
    /// <returns></returns>
    public static List<SugarEntityTypeDto> ReflexGetAllTEntityList()
    {
        ...
        entityTypeList = types.Select(type => new SugarEntityTypeDto(type.Name,
    ...
    /// </summary>
    /// <returns></returns>
    public static SugarEntityTypeDto ReflexGetAllTEntity(string name)
    {
        var entityType = ReflexGetAllTEntityList().FirstOrDefault(f => f.ClassName == name);
        if (entityType == null)
        {
            throw Oops.Oh(SugarErrorCode.SugarModelError);
        }
