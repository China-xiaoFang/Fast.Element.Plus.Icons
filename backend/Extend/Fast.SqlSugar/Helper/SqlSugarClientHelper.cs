...
static class SqlSugarClientHelper
{
    ...
    public static SysTenantDataBaseModel GetDbInfo(ISqlSugarClient _db, SysDataBaseTypeEnum dbType, long tenantId)
    {
        ...
        // 数据库信息缓存
        var dbInfoList = _cache.Get<List<SysTenantDataBaseModel>>($"{SugarCacheConst.CACHE_KEY_TENANT_DB_INFO}{tenantId}");
        if (dbInfoList == null || !dbInfoList.Any())
        {
            ...
            _cache.Set($"{SugarCacheConst.CACHE_KEY_TENANT_DB_INFO}{tenantId}", dbInfoList);
        }

        var db = dbInfoList.FirstOrDefault(f => f.SysDbType == dbType);
        if (db == null)
        {
            throw Oops.Oh(SugarErrorCode.TenantDbError);
        }
