...
using Fast.MiniExcel.AttributeFilter;
using Fast.MiniExcel.Dto;
using MiniExcelLibs;

namespace Fast.MiniExcel.Extension;
...
public static class Extension
{
    ...
    private static string ExportTypeDispose(MiniExcelColumn attr, PropertyInfo property, object value)
    {
        ...
        else if (property.PropertyType.IsEnum || value is Enum)
        {
            ...
            result = enumVal.GetType().GetMember(enumVal.ToString()).FirstOrDefault()?.GetCustomAttribute<DescriptionAttribute>()
                ?.Description ?? enumVal.ToString();
        }
        else
        {
            switch (property.PropertyType.Name)
            {
                // 判断是否为Boolean类型
                case "Boolean":
                    var boolVal = bool.Parse(value.ToString() ?? string.Empty);
                    if (boolVal)
        ...
        this IEnumerable<IDictionary<string, object>> excelData)
    {
        ...
        foreach (var (item, index) in excelList.Where(wh => wh.index != 0))
        {
            ...
            foreach (var property in properties)
            {
                ...
                // 查找当前属性的描述
                var proDesc = columnAttr == null ? proName :
    ...
    {
        return stream.Query().Cast<IDictionary<string, object>>().GetMiniExcelImportData<T>();
    }
    ...
    {
        return (await stream.QueryAsync()).Cast<IDictionary<string, object>>().GetMiniExcelImportData<T>();
    }
}