...
/// </summary>
public class User : AuthUserInfo, IUser, IScoped
{
    /// <summary>
    /// 当前租户Id
    /// </summary>
    ...
    public User(ICache cache)
    {
        ...
        var tenantID = httpContext.Request.Headers["TenantID"].ToString().Base64ToString().ParseToLong();
        ...
        // 判断是否不需要登录
        if (httpContext.GetMetadata<AllowAnonymousAttribute>() != null)
            return;

        // 由于生命周期的问题，这里的Token不能直接获取
        var token = JWTEncryption.GetJwtBearerToken(httpContext as DefaultHttpContext);
