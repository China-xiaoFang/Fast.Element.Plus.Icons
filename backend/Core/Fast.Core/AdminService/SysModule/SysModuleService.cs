...
public class SysModuleService : ISysModuleService, ITransient
{
    ...
    {
        // 从缓存中读取
        var result = await _cache.GetAsync<List<SysModuleModel>>($"{CacheConst.AuthModule}{_user.UserId}");
        ...
        // 放入缓存
        await _cache.SetAsync($"{CacheConst.AuthModule}{_user.UserId}", result);
