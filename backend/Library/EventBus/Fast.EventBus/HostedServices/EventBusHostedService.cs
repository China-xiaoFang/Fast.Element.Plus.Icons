...
using Fast.EventBus.FriendlyException;
...
internal sealed class EventBusHostedService : BackgroundService
{
    ...
    public EventBusHostedService(ILogger<EventBusService> logger, IServiceProvider serviceProvider,
        IEventSourceStorer eventSourceStorer, IEnumerable<IEventSubscriber> eventSubscribers, bool useUtcTimestamp,
        bool fuzzyMatch, bool gcCollect, bool logEnabled)
    {
        _logger = logger;
            ...
            foreach (var eventHandlerMethod in eventHandlerMethods)
            {
                ...
                foreach (var eventSubscribeAttribute in eventSubscribeAttributes)
                {
                    var wrapper = new EventHandlerWrapper(eventSubscribeAttribute.EventId)
                    {
                        ...
                        Pattern =
                            CheckIsSetFuzzyMatch(eventSubscribeAttribute.FuzzyMatch)
                                ? new Regex(eventSubscribeAttribute.EventId, RegexOptions.Singleline)
                                : default,
            ...
            taskFactory.StartNew(async () =>
            {
                ...
                var eventHandlerExecutingContext =
                    new EventHandlerExecutingContext(eventSource, properties, eventHandlerThatShouldRun.HandlerMethod,
                        eventSubscribeAttribute) {ExecutingTime = UseUtcTimestamp ? DateTime.UtcNow : DateTime.Now};
                ...
                try
                {
                    ...
                    if (Executor == default)
                    {
                        ...
                        await Retry.InvokeAsync(
                            ...
                            {
                                // 输出重试日志
                                _logger.LogWarning("Retrying {times}/{total} times for {EventId}", times, total,
                                    eventSource.EventId);
                            });
                    }
                    else
                ...
                catch (Exception ex)
                {
                    ...
                    // 标记异常
                    executionException =
                        new InvalidOperationException(string.Format("Error occurred executing in {0}.", eventSource.EventId), ex);
                ...
                finally
                {
                    ...
                    if (Monitor != default)
                    {
                        ...
                        var eventHandlerExecutedContext =
                            ...
                            {
                                ExecutedTime = UseUtcTimestamp ? DateTime.UtcNow : DateTime.Now, Exception = executionException
                            };
    ...
    private void ManageEventSubscribers(EventSubscribeOperateSource subscribeOperateSource)
    {
        ...
        if (subscribeOperateSource.Operate == EventSubscribeOperates.Append)
        {
            var wrapper = new EventHandlerWrapper(eventId)
            {
                ...
                Pattern =
                    CheckIsSetFuzzyMatch(subscribeOperateSource.Attribute?.FuzzyMatch)
                        ? new Regex(eventId, RegexOptions.Singleline)
                        : default,
    ...
    {
        return fuzzyMatch == null ? FuzzyMatch : Convert.ToBoolean(fuzzyMatch);
    }
    ...
    {
        return gcCollect == null ? GCCollect : Convert.ToBoolean(gcCollect);
    }
