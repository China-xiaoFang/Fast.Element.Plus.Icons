...
public static class Oops
{
    ...
    /// <param name="args">String.Format 参数</param>
    /// <returns>异常实例</returns>
    public static AppFriendlyException Oh<TException>(string errorMessage, params object[] args) where TException : class
    {
    ...
    {
        var (ErrorCode, Message) = GetErrorCodeMessage(errorCode, args);
        return new AppFriendlyException(Message, errorCode, Activator.CreateInstance(exceptionType, Message) as Exception)
        {
            ErrorCode = ErrorCode
        };
    }
    ...
    /// <param name="args">String.Format 参数</param>
    /// <returns>异常实例</returns>
    public static AppFriendlyException Oh<TException>(object errorCode, params object[] args) where TException : class
    {
    ...
    private static (object ErrorCode, string Message) GetErrorCodeMessage(object errorCode, params object[] args)
    {
        ...
        // 获取当前状态码匹配异常特性
        var ifExceptionAttribute = methodIfException?.IfExceptionAttributes?.FirstOrDefault(u =>
            u.ErrorCode != null && HandleEnumErrorCode(u.ErrorCode).ToString().Equals(errorCode.ToString()));
        ...
        {
            errorCodeMessage = methodIfException?.IfExceptionAttributes
                ?.FirstOrDefault(u => u.ErrorCode == null && !string.IsNullOrWhiteSpace(u.ErrorMessage))?.ErrorMessage;
        }

        // 字符串格式化
        return (errorCode,
            MontageErrorMessage(errorCodeMessage, errorCode.ToString(),
    ...
    private static object HandleEnumErrorCode(object errorCode)
    {
        ...
        {
            var fieldinfo = errorType.GetField(System.Enum.GetName(errorType, errorCode));
    ...
    private static IEnumerable<Type> GetErrorCodeTypes()
    {
        ...
        var errorCodeTypes = App.EffectiveTypes.Where(u => u.IsDefined(typeof(ErrorCodeTypeAttribute), true) && u.IsEnum);
    ...
    private static ConcurrentDictionary<string, string> GetErrorCodeMessages()
    {
        ...
        var errorCodeMessages = ErrorCodeTypes
            .SelectMany(u => u.GetFields().Where(u => u.IsDefined(typeof(ErrorCodeItemMetadataAttribute))))
            .Select(u => GetErrorCodeItemInformation(u)).ToDictionary(u => u.Key.ToString(), u => u.Value);
        ...
        {
            // 获取所有参数大于1的配置
            var fitErrorCodes = errorCodeMessageSettings.Definitions.Where(u => u.Length > 1)
    ...
    private static MethodIfException GetEndPointExceptionMethod()
    {
        try
        {
            ...
            var ifExceptionAttributes = stackTrace
                .Where(u => u.MethodInfo.MethodBase != null &&
                            u.MethodInfo.MethodBase.IsDefined(typeof(IfExceptionAttribute), true)).SelectMany(u =>
                    u.MethodInfo.MethodBase.GetCustomAttributes<IfExceptionAttribute>(true));

            // 组装方法异常对象
            methodIfException = new MethodIfException {ErrorMethod = errorMethod, IfExceptionAttributes = ifExceptionAttributes};
    ...
    private static (object Key, string Value) GetErrorCodeItemInformation(FieldInfo fieldInfo)
    {
        ...
        return (errorCodeItemMetadata.ErrorCode ?? fieldInfo.Name,
            errorCodeItemMetadata.ErrorMessage.Format(errorCodeItemMetadata.Args));
    }
