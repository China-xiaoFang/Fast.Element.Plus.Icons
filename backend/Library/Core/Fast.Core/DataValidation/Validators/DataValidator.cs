...
public static class DataValidator
{
    ...
    static DataValidator()
    {
        ...
        GetValidationTypeValidationItemMetadataCached =
            new ConcurrentDictionary<object, (string, ValidationItemMetadataAttribute)>();
    }
    ...
    public static DataValidationResult TryValidateObject(object obj, bool validateAllProperties = true)
    {
        ...
        if (obj.GetType().IsDefined(typeof(NonValidationAttribute), true))
            return new DataValidationResult {IsValid = true, MemberOrValue = obj};
        ...
        // 返回验证结果
        return new DataValidationResult {IsValid = isValid, ValidationResults = results, MemberOrValue = obj};
    }
    ...
    public static DataValidationResult TryValidateValue(object value, params ValidationAttribute[] validationAttributes)
    {
        ...
        // 返回验证结果
        return new DataValidationResult {IsValid = isValid, ValidationResults = results, MemberOrValue = value};
    }
        ...
        params object[] validationTypes)
    {
        ...
        if (value == null)
        {
            ...
            // 返回验证结果
            return new DataValidationResult {IsValid = false, ValidationResults = results, MemberOrValue = value};
        }
        ...
        foreach (var validationType in validationTypes)
        {
            ...
            // 验证结果
            var validResult = TryValidateValue(value, validationItemMetadata.RegularExpression,
                validationItemMetadata.RegexOptions);
            ...
            if (!validResult)
            {
                if (isValid != false)
                    isValid = false;
                // 添加错误消息
                results.Add(new ValidationResult(string.Format(validationItemMetadata.DefaultErrorMessage.Render(), value,
                    validationName)));
            }
        }

        // 返回验证结果
        return new DataValidationResult {IsValid = isValid ?? true, ValidationResults = results, MemberOrValue = value};
    }
        ...
        GetValidationTypeValidationItemMetadata(object validationType)
    {
        ...
        static (string, ValidationItemMetadataAttribute) Function(object validationType)
        {
            ...
            // 获取对应的枚举名称
            var validationName = System.Enum.GetName(type, validationType);
    ...
    private static IEnumerable<Type> GetValidationMessageTypes()
    {
        ...
        var validationMessageTypes =
            App.EffectiveTypes.Where(u => u.IsDefined(typeof(ValidationMessageTypeAttribute), true) && u.IsEnum);
    ...
    private static ConcurrentDictionary<string, ValidationItemMetadataAttribute> GetValidationValidationItemMetadatas()
    {
        ...
        var customErrorMessages = ValidationMessageTypes
            .SelectMany(u => u.GetFields().Where(u => u.IsDefined(typeof(ValidationMessageAttribute)))).ToDictionary(u => u.Name,
                u => u.GetCustomAttribute<ValidationMessageAttribute>().ErrorMessage.Render());
        ...
        // 获取所有验证属性
        var validationFields = ValidationTypes
            .SelectMany(u => u.GetFields().Where(u => u.IsDefined(typeof(ValidationItemMetadataAttribute))))
