...
public class MiniProgramUtil
{
    ...
    public static MiniProgramTelUserInfoDto AESDecrypt(string encryptedDataStr, string session_key, string iv)
    {
        var rlt = BasicAESDecrypt(encryptedDataStr, session_key, iv);
        return !rlt.IsEmpty() ? JSON.Deserialize<MiniProgramTelUserInfoDto>(rlt) : null;
    }
    ...
    private static string BasicAESDecrypt(string encryptedDataStr, string session_key, string iv)
    {
        ...
        managed.Key = Convert.FromBase64String(session_key);
...
/// 小程序登录解密后的用户信息Model
/// </summary>
public class MiniProgramTelUserInfoDto
{
    /// <summary>
    /// 手机号
    /// </summary>
