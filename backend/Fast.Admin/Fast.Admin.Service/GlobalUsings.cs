/*
 * Author 1.8K仔
 * Fast.Admin.Service 常用Using引用
 * 此文件为了减少代码，将所有的Using引用，全部放在此处
 */

global using System.ComponentModel.DataAnnotations;
global using Furion;
global using Furion.DependencyInjection;
global using Furion.FriendlyException;
global using Mapster;
global using SqlSugar;
global using Fast.Core;
global using Fast.Core.AdminFactory.EnumFactory;
global using Fast.Core.Const;
global using Fast.Core.Internal.Restful.Extension;
global using Fast.Core.Internal.Restful.Internal;
global using Fast.Core.Operation.Config;
global using Fast.Core.Util.Http;
global using Fast.Iaas.Extension;
global using Fast.Iaas.Util;
global using Fast.SqlSugar.Tenant.BaseModel.Dto;
global using Fast.SqlSugar.Tenant.Extension;
global using Fast.SqlSugar.Tenant.Helper;
global using Fast.SqlSugar.Tenant.Internal.Enum;
global using Fast.SqlSugar.Tenant.Repository;
global using Fast.SqlSugar.Tenant.SugarEntity;