...
public class SqlSugarEntityHandler : ISqlSugarEntityHandler
{
    ...
    public SqlSugarEntityHandler(ICache cache,ISqlSugarClient sqlSugarClient,IHttpContextAccessor httpContextAccessor,ILogger<SqlSugarEntityHandler> logger)
    {
        _cache = cache;
    ...
    {
        // 获取系统日志库连接字符串配置
        var logConnectionSettings = await 
            GetCacheConnectionSettings(FastDbTypeEnum.SysCoreLog, SystemConst.DefaultSystemTenantId, YesOrNotEnum.Y);
    ...
    /// <returns></returns>
    public async Task ExecuteTimeoutAsync(string fileName, int fileLine, string methodName, string sql, SugarParameter[] parameters,
        TimeSpan executionTime, string handlerSql, string message)
    {
    }
    ...
    /// <param name="message"><see cref="T:System.String" /></param>
    /// <returns></returns>
    public async Task ExecuteErrorAsync(string fileName, int fileLine, string methodName, string rawSql, SugarParameter[] parameters,
