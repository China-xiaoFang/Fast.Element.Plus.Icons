...
public static class HttpNewUtil
{
    ...
    {
        foreach (var hostAddress in Dns.GetHostAddresses(GetWebRemoteIp()))
        {
    ...
    public static string Url
    {
        get
        {
            var url = new StringBuilder().Append(HttpContext?.Request.Scheme).Append("://").Append(HttpContext?.Request.Host)
                .Append(HttpContext?.Request.PathBase).Append(HttpContext?.Request.Path).Append(HttpContext?.Request.QueryString)
                .ToString();
            return url;
        }
    }
