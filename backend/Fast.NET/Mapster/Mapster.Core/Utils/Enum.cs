...
{
    private static readonly Dictionary<string, TEnum> _lookup =
        ((TEnum[]) Enum.GetValues(typeof(TEnum))).ToDictionary(x => x.ToString().ToUpperInvariant(), y => y);

    private static readonly Dictionary<TEnum, string> _reverseLookup =
        ((TEnum[]) Enum.GetValues(typeof(TEnum))).ToDictionary(x => x, y => y.ToString());
    ...
    static Enum()
    {
        ...
        _toIntString = Expression.Lambda<Func<TEnum, string>>(
            Expression.Call(Expression.Convert(p3, _underlyingType), toString), p3).Compile();
        var parse = _underlyingType.GetMethod("Parse", new[] {typeof(string)});
        var p4 = Expression.Parameter(typeof(string));
        _toIntEnum = Expression.Lambda<Func<string, TEnum>>(Expression.Convert(Expression.Call(parse, p4), typeof(TEnum)), p4)
            .Compile();
    ...
    public static TEnum Parse(string val)
    {
        ...
        var ret = val.Split(',').Select(MatchByString).Aggregate(0ul, (a, b) => a | _toUlong(b));
        return _toEnum(ret);
    }
