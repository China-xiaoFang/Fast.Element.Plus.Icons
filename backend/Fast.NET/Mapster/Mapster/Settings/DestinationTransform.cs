...
public class DestinationTransform
{
    ...
    {
        Condition = type => type.IsArray || type.IsCollectionCompatible() || type.GetDictionaryType() != null,
        TransformFunc = type =>
        {
            Expression newExp;
            if (type.IsArray)
    ...
    public static readonly DestinationTransform CreateNewIfNull = new DestinationTransform
    {
        Condition = type =>
        {
            ...
            if (type.GetTypeInfo().IsInterface || type.GetTypeInfo().IsAbstract)
                return false;
            var ci = type.GetConstructors(BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic)
        ...
        {
            var p = Expression.Parameter(type);
            return Expression.Lambda(Expression.Coalesce(p, Expression.New(type)), p);
        }
    };
    ...
    {
        return new DestinationTransform {Condition = condition, TransformFunc = TransformFunc};
    }
}