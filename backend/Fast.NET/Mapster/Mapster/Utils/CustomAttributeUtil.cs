...
internal static class CustomAttributeUtil
{
    ...
    private static bool IsInheritable(this CustomAttributeData attr)
    {
        return attr.GetAttributeType().GetCustomAttributesData(true)
            .Where(it => it.GetAttributeType() == typeof(AttributeUsageAttribute)).SelectMany(it => it.NamedArguments).Any(it =>
                it.GetMemberName() == nameof(AttributeUsageAttribute.Inherited) && true.Equals(it.TypedValue.Value));
    }
