...
internal static class SpecificationDocumentBuilder
{
    ...
    public static SpecificationOpenApiInfo GetGroupOpenApiInfo(string group)
    {
        ...
        static SpecificationOpenApiInfo Function(string group)
        {
            // 替换路由模板
            var routeTemplate =
                _specificationDocumentSettings.RouteTemplate.Replace("{documentName}", Uri.EscapeDataString(group));
            ...
            if (App.Configuration.GetSection(string.Format(groupKey, group)).Exists())
            {
                ...
                SetProperty<string>(group, nameof(SpecificationOpenApiInfo.RouteTemplate),
                    value => groupInfo.RouteTemplate = value);
                ...
                SetProperty<Uri>(group, nameof(SpecificationOpenApiInfo.TermsOfService),
                    value => groupInfo.TermsOfService = value);
    ...
    internal static void BuildGen(SwaggerGenOptions swaggerGenOptions, Action<SwaggerGenOptions> configure = null)
    {
        ...
        if (_specificationDocumentSettings.EnableEnumSchemaFilter == true)
    ...
    private static void ConfigureSecurities(SwaggerGenOptions swaggerGenOptions)
    {
        ...
        if (_specificationDocumentSettings.EnableAuthorized != true ||
            _specificationDocumentSettings.SecurityDefinitions.Length == 0)
            return;
    ...
    private static void CustomizeIndex(SwaggerUIOptions swaggerUIOptions)
    {
        ...
        // 添加登录信息配置
        var additional = _specificationDocumentSettings.LoginInfo;
        if (additional != null)
        {
            swaggerUIOptions.ConfigObject.AdditionalItems.Add(nameof(_specificationDocumentSettings.LoginInfo), additional);
        }
    }
