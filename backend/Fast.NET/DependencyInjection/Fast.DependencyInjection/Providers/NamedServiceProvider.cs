...
internal class NamedServiceProvider<TService> : INamedServiceProvider<TService> where TService : class
{
    ...
    //{
    //    // 解析所有实现
    //    return _serviceProvider.GetServices<TService>().FirstOrDefault(u => u.GetType().Name == serviceName);
    //}
    ...
    public TService GetService<ILifetime>(string serviceName) where ILifetime : IDependency
    {
        ...
        return resolveNamed?.Invoke(serviceName, default) as TService;
    }
    ...
    //{
    //    // 解析所有实现
    //    var service = _serviceProvider.GetServices<TService>().FirstOrDefault(u => u.GetType().Name == serviceName);
