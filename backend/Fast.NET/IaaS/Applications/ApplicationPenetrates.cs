...
/// <summary>
/// <see cref="ApplicationPenetrates"/> 内部Api 常量，公共方法配置类
/// </summary>
internal static class ApplicationPenetrates
{
    /// <summary>
    /// 控制器排序集合
    /// </summary>
    ...
    public static bool IsApiController(Type type)
    {
        ...
        static bool Function(Type type)
        {
            ...
            var iApplicationType = typeof(IApplication);
            ...
            if ((!typeof(Controller).IsAssignableFrom(type) && typeof(ControllerBase).IsAssignableFrom(type)) ||
                iApplicationType.IsAssignableFrom(type))
            {
                return true;
            }
