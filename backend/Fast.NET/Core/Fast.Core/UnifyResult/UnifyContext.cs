...
namespace Fast.Core.UnifyResult;
...
public static class UnifyContext
{
    ...
    public static ExceptionMetadata GetExceptionMetadata(ActionContext context)
    {
        ...
        var exception = context is ExceptionContext exContext
            ? exContext.Exception
            : (context is ActionExecutedContext edContext ? edContext.Exception : default);
        ...
        if (!isValidationException)
        {
            ...
            var ifExceptionAttributes = context.ActionDescriptor is not ControllerActionDescriptor actionDescriptor
                ...
                : actionDescriptor.MethodInfo.GetCustomAttributes<IfExceptionAttribute>(true).Where(u => u.ErrorCode == null);
            ...
            if (ifExceptionAttributes.Any())
            {
                ...
                var actionIfExceptionAttribute = ifExceptionAttributes.FirstOrDefault(u => u.ExceptionType ==
                                                     (isFriendlyException && exception?.InnerException != null
                                                         ...
                                                         : exception?.GetType())) ??
                                                 ifExceptionAttributes.FirstOrDefault(u =>
                                                     u.ExceptionType == typeof(Exception)) ??
            ...
            else
            {
                ...
                errors = exception?.InnerException?.Message ??
                         exception?.Message ?? friendlyExceptionSettings.DefaultErrorMessage;
            }
        }
    ...
    public static void Fill(object extras)
    {
        ...
        if (items.ContainsKey(UnifyResultExtrasKey))
    ...
    public static object GetSerializerSettings(FilterContext context)
    {
        ...
        // 获取序列化配置
        var unifySerializerSettingAttribute =
            actionDescriptor.MethodInfo.GetFoundAttribute<UnifySerializerSettingAttribute>(true);
        if (unifySerializerSettingAttribute == null || string.IsNullOrWhiteSpace(unifySerializerSettingAttribute.Name))
            return null;
    ...
    internal static bool CheckSucceededNonUnify(MethodInfo method, out IUnifyResultProvider unifyResult, bool isWebRequest = true)
    {
        ...
        // 判断是否跳过规范化处理
        var isSkip = !EnabledUnifyHandler || method.GetRealReturnType().HasImplementedRawGeneric(unityMetadata.ResultType) ||
                     method.CustomAttributes.Any(x =>
                     ...
                     method.ReflectedType.IsDefined(typeof(NonUnifyAttribute), true) || method.DeclaringType.Assembly.GetName()
    ...
    internal static bool CheckStatusCodeNonUnify(HttpContext context, out IUnifyResultProvider unifyResult)
    {
        ...
        // 判断是否跳过规范化处理
        var isSkip = !EnabledUnifyHandler || context.GetMetadata<NonUnifyAttribute>() != null ||
                     endpointFeature?.Endpoint?.Metadata?.GetMetadata<NonUnifyAttribute>() != null ||
                     context.Request.Headers["accept"].ToString()
                         .Contains("odata.metadata=", StringComparison.OrdinalIgnoreCase) || context.Request.Headers["accept"]
                         .ToString().Contains("odata.streaming=", StringComparison.OrdinalIgnoreCase);
        ...
        out UnifyResultSettingsOptions unifyResultSettings)
    {
        ...
        // 如果未启用 MVC 规范化处理，则跳过
        if (unifyResultSettings?.SupportMvcController == false &&
            typeof(Controller).IsAssignableFrom(actionDescriptor.ControllerTypeInfo))
            return false;
