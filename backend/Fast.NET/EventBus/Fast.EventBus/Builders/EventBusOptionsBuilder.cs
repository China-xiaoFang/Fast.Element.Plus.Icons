...
using Fast.EventBus.Executors;
using Microsoft.Extensions.DependencyInjection;
using Microsoft.Extensions.DependencyInjection.Extensions;

namespace Fast.EventBus.Builders;
...
public sealed class EventBusOptionsBuilder
{
    ...
    public EventBusOptionsBuilder AddSubscriber<TEventSubscriber>() where TEventSubscriber : class, IEventSubscriber
    {
    ...
    {
        // 类型检查
        if (!typeof(IEventSubscriber).IsAssignableFrom(eventSubscriberType) || eventSubscriberType.IsInterface)
    ...
    public EventBusOptionsBuilder AddSubscribers(params Assembly[] assemblies)
    {
        ...
        var subscribers = assemblies.SelectMany(ass => ass.GetExportedTypes().Where(t =>
            t.IsPublic && t.IsClass && !t.IsInterface && !t.IsAbstract && typeof(IEventSubscriber).IsAssignableFrom(t)));
    ...
    /// <returns><see cref="EventBusOptionsBuilder"/> 实例</returns>
    public EventBusOptionsBuilder ReplacePublisher<TEventPublisher>() where TEventPublisher : class, IEventPublisher
    {
    ...
    {
        // 空检查
        if (createStorer == null)
            throw new ArgumentNullException(nameof(createStorer));
    ...
    /// <returns><see cref="EventBusOptionsBuilder"/> 实例</returns>
    public EventBusOptionsBuilder AddMonitor<TEventHandlerMonitor>() where TEventHandlerMonitor : class, IEventHandlerMonitor
    {
    ...
    /// <returns><see cref="EventBusOptionsBuilder"/> 实例</returns>
    public EventBusOptionsBuilder AddExecutor<TEventHandlerExecutor>() where TEventHandlerExecutor : class, IEventHandlerExecutor
    {
    ...
    {
        // 类型检查
        if (!typeof(IEventFallbackPolicy).IsAssignableFrom(fallbackPolicyType) || fallbackPolicyType.IsInterface)
            throw new InvalidOperationException("The <fallbackPolicyType> is not implement the IEventFallbackPolicy interface.");
