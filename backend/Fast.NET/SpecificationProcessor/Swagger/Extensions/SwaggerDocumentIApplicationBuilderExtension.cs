...
/// </summary>
public static class SwaggerDocumentIApplicationBuilderExtension
{
    /// <summary>
    /// 添加Swagger文档中间件
    /// </summary>
        ...
        Action<SwaggerOptions> configureSwagger = default, Action<SwaggerUIOptions> configureSwaggerUI = default)
    {
        ...
        // 配置 Swagger 全局参数
        app.UseSwagger(options => SwaggerDocumentBuilder.Build(options, configureSwagger));

        // 配置 Swagger UI 参数
        app.UseSwaggerUI(options => SwaggerDocumentBuilder.BuildUI(options, configureSwaggerUI));
