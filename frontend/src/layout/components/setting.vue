		...
		<div class="scrollbar">
			...
			<div class="snowy-setting-checkbox">
				<a-tooltip v-for="(a, i) in sideStyleList" :key="i" placement="top">
					<template #title><span>{{ a.tips }}</span></template>
			...
			<div :style="{ marginBottom: '24px' }">
				<span>
					<h4>
						顶栏主题色通栏：<a-switch style="float: right" v-model:checked="topHanderThemeColorSpread"
							:disabled="!topHanderThemeColorOpen" /></h4>
				</span>
			</div>
			<a-divider />
			...
			<a-alert message="以上配置可实时预览，开发者可在 config/index.js 中配置默认值，不建议在生产环境下开放布局设置" type="warning" />
		</div>
	</div>
</template>
